...
public class MultipleChoiceHorizontal extends QuestionType {
  ...
  public void getAnswer() {
    ...
    for (int i = 0; i < answer.length(); i++) {
      if (answer.charAt(i) == '*') {
        for (int j = i + 1; answer.charAt(j) != ',' && j < answer.length(); j++) {
          input += answer.charAt(j);
        }
        answer.erase(i, 1);
      }
    }
  ...
  public void getOptions() {
    ...
    for (int i = 0; i <= answer.length(); i++) {
      if (answer.charAt(i) == ',' || i == answer.length()) {
        appendString(replace, "<label>", temp, option,  "'>", option, "</label>");
        option = "";
        count++;
      } 
      else {
        option += answer.charAt(i);
      }
    }
    count = 1;
    replace += "\n</div>";
