...
public:
    ...
    virtual string print(const LiQuizCompiler* compiler, ostream &a, int &pN, int &qN, double &p) = 0;
...
public:
    ...
    string print(const LiQuizCompiler* compiler, ostream &answersFile, int &partNum, int &questionNum, double &points)
    {
        input = "";
        answer = text;
        smatch m;
...
{
public:
    string print(const LiQuizCompiler*, ostream &answersFile, int &partNum, int &questionNum, double &points)
    {
        text.erase(0, 4);
...
private:
    ...
    void findDefinitions(const string &name, string &defs) const
    {
...
public:
    ...
    void makeQuestion(nlohmann::json &question)
    {
        ...
        if (style != "def")
        {
            ...
            html << "<div class='q' id='q" << questionNum << "'>" << questionNum << ". " << questionName << "<span class='pts'> (" << points << " points)</span></p>";
            html << "\n"
                 << preStart << "\n";
            smatch m;
    ...
    void grabQuestions()
    {
        ...
        {
            if (regex_search(line, m, questionStart))
            { // looking for the beginning of a question
...
string Definition::print(const LiQuizCompiler* compiler, ostream &answersFile, int &partNum, int &questionNum, double &points)
{
    answer = text.erase(0, 4);
